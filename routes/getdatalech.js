...
function readData(callback) {
    var ansverAll = [];
    request({url: currencyUrlLechInoCont, encoding: null}, function (error, response, body) {
         ...
         */
            ...
            *Конец
            */
            ansverAll.push({lechinocont: ansverSort});
            //console.log(ansver);
            //callback(ansverSort);
            request({url: currencyUrlLechRFBug, encoding: null}, function (error, response, body) {
                if (!error && response.statusCode === 200) {
                    var $ = cheerio.load(translator.convert(body).toString());
                    cheerioTableparser($);
                    var table = $('table').parsetable(true, true, true);
         ...
         */
                    ...
                    *Конец
                    */
                    ansverAll.push({lechrfbug: ansverSort});
                    request({url: currencyUrlLechRFCont, encoding: null}, function (error, response, body) {
                        if (!error && response.statusCode === 200) {
                            var $ = cheerio.load(translator.convert(body).toString());
                            cheerioTableparser($);
                            var table = $('table').parsetable(true, true, true);
                            ...
                            *Конец
                            */
                            ansverAll.push({lechrfcont: ansverSort});
