...
function readData(callback) {
    var ansverAll = [];
    request({url: currencyUrlStomInoCont, encoding: null}, function (error, response, body) {
        ...
        */
            ...
            *Конец
            */
            ansverAll.push({stominocont: ansverSort});
            //console.log(ansver);
            //callback(ansverSort);
            request({url: currencyUrlStomRFBug, encoding: null}, function (error, response, body) {
                if (!error && response.statusCode === 200) {
                    var $ = cheerio.load(translator.convert(body).toString());
                    cheerioTableparser($);
                    var table = $('table').parsetable(true, true, true);
        ...
        */
                    ...
                    *Конец
                    */
                    ansverAll.push({stomrfbug: ansverSort});
                    request({url: currencyUrlStomRFCont, encoding: null}, function (error, response, body) {
                        if (!error && response.statusCode === 200) {
                            //Парсим таблицу
                            var $ = cheerio.load(translator.convert(body).toString());
                            cheerioTableparser($);
                          ...
                          //Формируем квоту, кол-во подавших и мин бал для прохождения
                            let a = 0, b = 0, vacansy;
                            ansverSort.push([]);
                            for (i = 0; i < table[0].length; i++) {
                                if (table[0][i] === 'Вакантных мест:'||table[0][i] === 'Вакантных\n  мест:') {
                            ...
                            ansverAll.push({stomrfcont: ansverSort});
